...
namespace OOP_LB4_1_3_task
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Human human = new Human("Vitya", "Yanukovich", "Male", 23);
            Debug.WriteLine($"This is class {human.GetType().Name}");
            human.Info();

            Employee employee = new Employee("Yuriy", "Pospelov", "Male", 18, "manager", 18785);
            ...
            Unit unit = new Unit("Frog", "Kiev");
            unit.ShowInfo();

            Hero hero = new Hero("Lucas", "bird", 1200, "Sword", 15);
            hero.ShowInfo();

            Enemy enemy = new Enemy("Pig", "Mountain", 20000, "Fire", 50);
            enemy.ShowInfo();
